"""
Nutrition app views - AI Chat va AI Insights endpoints
"""
import json
import requests
...
- O'zbek taomlari haqida bilim (osh, so'msa, lag'mon, shashlik, non)
- Ilmiy asoslangan maslahatlar
- Kontekstdagi ma'lumotlarga asoslanib javob ber
"""
...
- Kunlik maqsad: {daily_goal} kkal

Bugun:
- Kaloriya: {consumed_cals:.0f}/{daily_goal} kkal
- Oqsil: {consumed_protein:.0f}/{protein_goal:.0f} g
- Uglevod: {consumed_carbs:.0f} g
- Yog': {consumed_fat:.0f} g
- Suv: {water}/2000 ml

Ovqatlar:
{chr(10).join(meal_details) if meal_details else 'Hali ovqat kiritilmagan'}

Oxirgi vazn: {f'{last_weight.weight_kg} kg' if last_weight else 'Kiritilmagan'}
"""
    return context
...
class AIChatView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({'error': 'AI javob bermadi'}, status=504)
        except requests.exceptions.RequestException as e:
            return Response({'error': f'AI xatolik: {str(e)}'}, status=502)
        except Exception as e:
            return Response({'error': f'Xatolik: {str(e)}'}, status=500)


class AIInsightsView(APIView):
    """AI Insights - foydalanuvchi ma'lumotlariga asoslangan xulosalar"""
    permission_classes = [IsAuthenticated]
        ...
        try:
            ...
            context = {
                "profile": {
                    ...
                    "current_weight": float(profile.weight_kg) if profile.weight_kg else 0,
                    ...
                    "bmr": float(profile.bmr) if profile.bmr else 0,
                    "tdee": float(profile.tdee) if profile.tdee else 0,
                },
                "targets": {
            ...
            return Response({
                ...
                'insights': {
                    ...
                    "recommendations": ["Profil va ovqat ma'lumotlarini kiriting"],
                    "warnings": [],
                    "motivation": "Boshlang!"
                }
            }, status=200)
    ...
    def _get_ai_insights(self, context):
        ...
        if not api_key:
            return {
                "summary": "AI xulosa uchun API key sozlanmagan",
                "recommendations": [],
                "warnings": [],
                "motivation": ""
            }
        ...
        prompt = f"""
Siz professional dietolog. Foydalanuvchi ma'lumotlarini tahlil qilib, qisqa xulosalar bering (o'zbek tilida).
...
**Oxirgi 7 kun:**
- Jurnal: {context['last_7_days']['days_logged']}/{context['last_7_days']['total_days']} kun
        ...
        try:
            response = requests.post(
                ...
                json={
                    'model': 'deepseek-chat',
                    'messages': [
                        {'role': 'system', 'content': 'Siz dietolog. Faqat JSON javob bering.'},
            ...
            return {
                ...
                "recommendations": ["Ma'lumotlarni to'liq kiriting"],
                "warnings": [],
